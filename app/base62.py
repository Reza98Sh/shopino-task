...
class Base62:
    ...
    def __init__(self, fixed_length: int = 4):
        ...
        self._char_to_index = {char: idx for idx, char in enumerate(self.ALPHABET)}
    ...
    def encode(self, num: int) -> str:
        ...
        if num <= 0:
            raise ValueError("Only non-negative and non-zero integers are supported.")
        else:
            parts = []
            while num > 0:
        ...
        if len(result) > self.fixed_length:
            raise ValueError(f"Number {num} is too large for fixed length {self.fixed_length}")
    ...
    def decode(self, encoded: str) -> int:
        ...
        :param encoded: Base-62 encoded string
